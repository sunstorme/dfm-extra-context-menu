...
move_built_packages() {
    ...
    # 创建packages目录及项目子目录
    local app_project_dir="$packages_dir/app/$project_name"
    local debug_project_dir="$packages_dir/debug/$project_name"
    local dev_project_dir="$packages_dir/dev/$project_name"

    mkdir -p "$app_project_dir" "$debug_project_dir" "$dev_project_dir"

    # 移动构建产物到对应分类目录
    log "移动构建产物到packages目录（项目: $project_name）..."
    local moved_count=0
    local app_count=0
    local debug_count=0
    local dev_count=0
    ...
    while IFS= read -r deb_file; do
        if [ -f "$deb_file" ]; then
            ...
            local target_dir="$app_project_dir"
            local pkg_type="普通包"

            # 判断包类型
            if [[ "$deb_name" =~ dbgsym ]]; then
                target_dir="$debug_project_dir"
                pkg_type="调试包"
                debug_count=$((debug_count + 1))
            elif [[ "$deb_name" =~ \-dev_ ]]; then
                target_dir="$dev_project_dir"
                pkg_type="开发包"
    ...
    success "  - 项目: $project_name"
    success "  - 普通包: $app_count 个 (app/$project_name/)"
    if [ $debug_count -gt 0 ]; then
        success "  - 调试包: $debug_count 个 (debug/$project_name/)"
    fi
    if [ $dev_count -gt 0 ]; then
        success "  - 开发包: $dev_count 个 (dev/$project_name/)"
    fi

    # 使用系统默认应用打开当前项目的 app 目录
    local open_dir="$app_project_dir"
...
show_built_packages() {
    ...
    # 从packages项目子目录获取当前构建产生的deb包
    local app_debs=$(find "$app_project_dir" -maxdepth 1 -name "*.deb" -mmin -2 2>/dev/null || true)
    local debug_debs=$(find "$debug_project_dir" -maxdepth 1 -name "*.deb" -mmin -2 2>/dev/null || true)
    local dev_debs=$(find "$dev_project_dir" -maxdepth 1 -name "*.deb" -mmin -2 2>/dev/null || true)

    # 如果没有找到最近的包，则尝试通过项目名筛选
    if [ -z "$app_debs" ]; then
        app_debs=$(find "$app_project_dir" -maxdepth 1 -name "*.deb" 2>/dev/null || true)
    fi
    if [ -z "$debug_debs" ]; then
        debug_debs=$(find "$debug_project_dir" -maxdepth 1 -name "*.deb" 2>/dev/null || true)
    fi
    if [ -z "$dev_debs" ]; then
        dev_debs=$(find "$dev_project_dir" -maxdepth 1 -name "*.deb" 2>/dev/null || true)
    fi

    # 显示分类构建产物
    if [ -n "$app_debs" ] || [ -n "$debug_debs" ] || [ -n "$dev_debs" ]; then
        log "构建产物 (项目: $project_name，已按类型分类):"
        echo ""

        # 显示普通应用包
        if [ -n "$app_debs" ]; then
            log "普通应用包 (app/$project_name/):"
            ls -lh $app_debs
            echo ""
        fi
...
构建产物:
    - DEB包: 按类型和项目名自动分类到packages目录的子目录
      * packages/app/<项目名>/     - 普通应用包
      * packages/debug/<项目名>/   - 调试符号包（*dbgsym，用于调试崩溃）
      * packages/dev/<项目名>/     - 开发包（*-dev，包含头文件和开发库）
